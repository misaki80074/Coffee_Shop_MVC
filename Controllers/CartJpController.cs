...
{
    public class CartJpController : Controller
    {
            private readonly DBCNcart _dbcn;
            private readonly ProjectContext _context;

            // 建構函數注入 DBCNcart 和 ProjectContext
            public CartJpController(DBCNcart dbcn, ProjectContext context)
            {
                _dbcn = dbcn;
                _context = context;
            }
            ...
            public IActionResult Index()
            {
                ...
                List<ProductJp> productList = new List<ProductJp>();
                foreach (DataRow row in dt.Rows)
                {
                    productList.Add(new ProductJp
                    {
...
// --------------------------------------------------------------------------------
        ...
        public async Task<IActionResult> DeleteCartItem([FromBody] SingelCartJson cart)
        {
            ...
            // 重導回購物車頁面
            return RedirectToAction("shopping_cart", "CartJp");
        }
        ...
        [HttpGet]
        [Route("/CartJp/RemoveCart/{userId}")]
