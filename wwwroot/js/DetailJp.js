...
function Select_Choose() {
    if ($('.cooffee-drink-select').val() === "other") {
        ...
        $('.coffee-drink-input').prop('placeholder', '整数を入力してください');
...
$(".btn-calculate").on('click', function () {
    ...
    if (input_value < 50 || input_value > 1000 || isNaN(input_value)) {
        // 輸入不符合條件，跳出警示框
        alert('エラー：50から1000までの整数を入力してください。\nお湯の量が多すぎると、最適な風味を損なう可能性があります。');
        $('.coffee-drink-input').val(''); // 清空輸入框
        $('.coffee-drink-input').prop('placeholder', '正しい数字をもう一度入れてください。');
        elements.forEach(function (element, index) {
...
function calculateParameters() {
    ...
    const baking_data = [
        { "baking": "ライト", "watertemperature": "90-92", "brewtime": "2:30-3:00" },
        { "baking": "シナモン", "watertemperature": "91-93", "brewtime": "2:15-2:45" },
        { "baking": "ミディアム", "watertemperature": "92-94", "brewtime": "2:00-2:30" },
        { "baking": "シティ", "watertemperature": "93-95", "brewtime": "1:45-2:15" },
        { "baking": "フレンチ", "watertemperature": "94-96", "brewtime": "1:30-2:00" },
    ];
    // 使用者輸入的咖啡飲用量
...
$.ajax({
    ...
    error: function () {
        $('#chatbot-body').append('<div class="message-bot">データの読み込みエラーです。しばらくお待ちください。</div>');
    }
});
scrollToBottom();
