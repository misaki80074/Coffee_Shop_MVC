...
$('.accordion a').on('click', function () {
    ...
    if (text == "すべて" || text == "ドリップコーヒー") {
        history.pushState({ pathname: "column" }, "", window.location.origin + `/ListJp/${text}`)
        getProData(getAjaxUrl())
    } else {
        var col_text = $(this).closest('.accordion-item').find('button').text()
        history.pushState({ pathname: "column" }, "", window.location.origin + `/ListJp/${col_text}/${text}`)
        getProData(getAjaxUrl())
    }
    setBreadcrumb()
})
...
$('.priceSort li').on('click', function () {
    ...
    if (text == "オススメ") {
        $(this).closest('div').find('button').text("並べ替え")
        deleteUrl("sort")
    } else if (text.includes("安い順")) {
        setUrl("sort", "desc")
    } else if (text.includes("高い順")) {
        setUrl("sort", "asc")
    }
})
...
function setBreadcrumb() {
    ...
    $('.breadcrumb').append('<li class="breadcrumb-item"><a href="/Home/Index">ホームページ</a></li>')
    if (pathname.length > 3) {
        ...
        $('.breadcrumb').append(`<li class="breadcrumb-item">すべて</li>`)
    }
}
...
function setSortNItems() {
    ...
    if (queryMap.get("sort") == null) {
        $('.priceSort li').closest('div').find('button').text("並べ替え")
    } else {
        if (queryMap.get("sort") == "desc") {
            $('.priceSort li').closest('div').find('button').text("価格:安い順")
        }
        else if (queryMap.get("sort") == "asc") {
            $('.priceSort li').closest('div').find('button').text("価格:高い順")
        }
    }
    if (queryMap.get("item") == null || queryMap.get("item") == "12") {
        $('.itemShow li').closest('div').find('button').text("1ページに12件")
    } else if (queryMap.get("item") == "18") {
        $('.itemShow li').closest('div').find('button').text("1ページに18件")
    }
}
...
function getAjaxUrl() {
    ...
    if (pathname.length < 4) {
        ajaxurl = window.location.origin + `/ListJp/QueryJp/${(pathname[2] != null) ? pathname[2] : "すべて"}` + q + queryMap
        return ajaxurl
    } else {
        pathname.splice(2, 0, "QueryJp")
...
function getProData(queryString) {
    $.ajax({
        ...
        success: function (jsonData) {
            $(".ItemSort > div:first").text(jsonData.totalCount + " 個")
            $('.cardContainer').empty()
            setPage()
                ...
                let doc =
                    ...
                    <div class="card">
                        <div class="cardImgBody">
                            ...
                            <div>
                                <a href="/DetailJp/DetailJp/${jsonData.products[i].id}">
                        ...
                        <div class="cardImgBody2">
                            <a href="/DetailJp/DetailJp/${jsonData.products[i].id}"><img src=${jsonData.products[i].img} class="card-img-top"></a>
                        </div>
                        <div class="card-body">
