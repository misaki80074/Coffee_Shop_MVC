...
$('.accordion a').on('click', function () {
    ...
    if (text == "All" || text == "Drip Bag") {
        history.pushState({ pathname: "column" }, "", window.location.origin + `/ListEn/${text}`)
        getProData(getAjaxUrl())
    } else {
        var col_text = $(this).closest('.accordion-item').find('button').text()
        history.pushState({ pathname: "column" }, "", window.location.origin + `/ListEn/${col_text}/${text}`)
        getProData(getAjaxUrl())
    }
    setBreadcrumb()
})
...
$('.priceSort li').on('click', function () {
    ...
    if (text == "Featured") {
        $(this).closest('div').find('button').text("Sort")
        deleteUrl("sort")
...
$('.filter input[type="checkbox"]').on('click', function () {
    ...
    if (colElem == "Roasting Grades") {
        setCheckboxUrl("baking", this)
    } else if (colElem == "Processing Methods") {
...
function setBreadcrumb() {
    ...
    $('.breadcrumb').append('<li class="breadcrumb-item"><a href="/Home/Index">Home Page</a></li>')
    if (pathname.length > 3) {
        ...
        $('.breadcrumb').append(`<li class="breadcrumb-item">All</li>`)
    }
}
...
function setSortNItems() {
    ...
    if (queryMap.get("sort") == null) {
        $('.priceSort li').closest('div').find('button').text("Sort")
    } else {
        if (queryMap.get("sort") == "desc") {
            $('.priceSort li').closest('div').find('button').text("Price: Low to High")
        }
        else if (queryMap.get("sort") == "asc") {
            $('.priceSort li').closest('div').find('button').text("Price: High to Low")
        }
    }
    if (queryMap.get("item") == null || queryMap.get("item") == "12") {
        $('.itemShow li').closest('div').find('button').text("12 items per page")
    } else if (queryMap.get("item") == "18") {
        $('.itemShow li').closest('div').find('button').text("18 items per page")
    }
}
...
function getAjaxUrl() {
    ...
    if (pathname.length < 4) {
        ajaxurl = window.location.origin + `/ListEn/QueryEn/${(pathname[2] != null) ? pathname[2] : "All"}` + q + queryMap
        return ajaxurl
    } else {
        pathname.splice(2, 0, "QueryEn")
...
function getProData(queryString) {
    $.ajax({
        ...
        success: function (jsonData) {
            $(".ItemSort > div:first").text(jsonData.totalCount + " items")
            $('.cardContainer').empty()
            setPage()
                ...
                let doc =
                    ...
                    <div class="card">
                        <div class="cardImgBody">
                            ...
                            <div>
                                <a href="/DetailEn/DetailEn/${jsonData.products[i].id}">
                        ...
                        <div class="cardImgBody2">
                            <a href="/DetailEn/DetailEn/${jsonData.products[i].id}"><img src=${jsonData.products[i].img} class="card-img-top"></a>
                        </div>
                        <div class="card-body">
